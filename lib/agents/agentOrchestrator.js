...
export const AGENT_REGISTRY = [
  ...
  },
  {
    ...
    role: "Controls Razorpay Smart Route custodial escrow, executes escrow payment, and disburses milestone payouts.",
    status: "ACTIVE",
...
export function getInterAgentEventStream(step, intent, auction, contract, escrow) {
  ...
  if (step >= 5 || escrow) {
    events.push({
      ...
      level: "FINANCE",
      message: `Razorpay Smart Route Escrow order initialized (${escrow?.orderId || "order_auctra_live"}). Custodial ledger: ₹${(contract?.totalAmount || 37000).toLocaleString("en-IN")}. Payout locked pending physical inspection gate.`
    });
  }
